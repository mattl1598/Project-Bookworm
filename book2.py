...
class Book:
	...
	def __init__(self, title=str("None"), author=str("None"), genre=str("None"), released=str("None"),
		...
		try:
			urllib.request.urlretrieve(img_url, assets+"image.jpg")
			img2gif.main()
			img2gif.resize()
			self.photo = ImageTk.PhotoImage(file=assets+"image.gif")
		except TypeError:
			img2gif.error()
			img2gif.resizeError()
			self.photo = ImageTk.PhotoImage(file=assets+"error.gif")
		except ValueError:
			# img2gif.error()
			# img2gif.resizeError()
			self.photo = ImageTk.PhotoImage(file=assets+"error.gif")
